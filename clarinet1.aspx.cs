...
public partial class clarinet1 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
		if (Request.Form["add"] != null)
		{
			...
			{
				string sql1 = "select clarinet1 from [Stats] where Uname ='" + Session["user"].ToString() + "'";
				...
				string sql2 = "update [Stats] set clarinet1 = " + num + " where Uname='" + Session["user"].ToString() + "'";
				...
				if (count == 0)
				{
					for (int i = 0; i < 9; i++)
					{
						...
						if (instrument == "")
						{
							sql4 = "update [Cart] set " + stArr[i] + "='clarinet1' where Uname = '" + Session["user"].ToString() + "'";
