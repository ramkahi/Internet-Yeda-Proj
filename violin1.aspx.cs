...
public partial class violin1 : System.Web.UI.Page
{
	protected void Page_Load(object sender, EventArgs e)
	{
		if (Request.Form["add"] != null)
		{
			...
			{
				string sql1 = "select violin1 from [Stats] where Uname ='" + Session["user"].ToString() + "'";
				string st = MyAdoHelper.DoQueryGetInstrument("Database.mdf", sql1, "violin1");
				if (st != " ")
				{
					num = Int32.Parse(st) + 1;
				}

				string sql2 = "update [Stats] set violin1 = " + num + " where Uname='" + Session["user"].ToString() + "'";
