...
public partial class eastrock_Tenor : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
		...
		if (Request.Form["add"] != null)
		{
			...
			{
				string sql1 = "select saxophone2 from [Stats] where Uname ='" + Session["user"].ToString() + "'";
				string st = MyAdoHelper.DoQueryGetInstrument("Database.mdf", sql1, "saxophone2");
				if (st != " ")
				{
					num = Int32.Parse(st) + 1;
				}

				string sql2 = "update [Stats] set saxophone2 = " + num + " where Uname='" + Session["user"].ToString() + "'";
				...
				if (count == 0)
				{
					for (int i = 0;i < 9;i++)
					{
						...
						if (instrument == "")
						{
							sql4 = "update [Cart] set " + stArr[i]+"='saxophone2' where Uname = '" + Session["user"].ToString() + "'";
