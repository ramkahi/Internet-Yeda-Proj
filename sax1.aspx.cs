...
public partial class Jean_Paul_Alto_Saxophone : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
		if (Request.Form["add"] != null)
		{
			...
			{
				string sql1 = "select saxophone1 from [Stats] where Uname ='" + Session["user"].ToString() + "'";
				string st = MyAdoHelper.DoQueryGetInstrument("Database.mdf", sql1, "saxophone1");
				if (st != " ")
				{
					num = Int32.Parse(st) + 1;
				}

				string sql2 = "update [Stats] set saxophone1 = " + num + " where Uname='" + Session["user"].ToString() + "'";
