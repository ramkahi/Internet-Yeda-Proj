...
public partial class clarinet2 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
		if (Request.Form["add"] != null)
		{
			...
			{
				string sql1 = "select clarinet2 from [Stats] where Uname ='" + Session["user"].ToString() + "'";
				...
				string sql2 = "update [Stats] set clarinet2 = " + num + " where Uname='" + Session["user"].ToString() + "'";
				...
				if (count == 0)
				{
					for (int i = 0; i < 9; i++)
					{
						...
						if (instrument == "")
						{
							sql4 = "update [Cart] set " + stArr[i] + "='clarinet2' where Uname = '" + Session["user"].ToString() + "'";
